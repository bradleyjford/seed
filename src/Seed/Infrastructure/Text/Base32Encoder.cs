...
namespace Seed.Infrastructure.Text
{
	public class Base32Encoder : Base32Encoding
	{
		...
		{
			var outputStringLength = Math.Max((int)Math.Ceiling(value.Length * BitsPerByte / (double)BitsPerCharacter), 1);
		...
		{
			for (var offset = ((availableBytes + 1) * BitsPerByte) - BitsPerCharacter; offset > 3; offset -= BitsPerCharacter)
			{
