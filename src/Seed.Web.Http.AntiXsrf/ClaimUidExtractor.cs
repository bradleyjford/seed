...
namespace Seed.Web.Http.AntiXsrf
{
    internal sealed class ClaimUidExtractor : IClaimUidExtractor
    {
        ...
        private readonly string _uniqueClaimTypeIdentifier;
        private readonly ClaimsIdentityConverter _claimsIdentityConverter;

        internal ClaimUidExtractor(string uniqueClaimTypeIdentifier, ClaimsIdentityConverter claimsIdentityConverter)
        {
            _uniqueClaimTypeIdentifier = uniqueClaimTypeIdentifier;
        ...
        public BinaryBlob ExtractClaimUid(IIdentity identity)
        {
            ...
            var claimsIdentity = _claimsIdentityConverter.TryConvert(identity);
            ...
            var uniqueIdentifierParameters = GetUniqueIdentifierParameters(claimsIdentity, _uniqueClaimTypeIdentifier);
            var claimUidBytes = CryptoUtil.ComputeSHA256(uniqueIdentifierParameters);
        ...
        internal static string[] GetUniqueIdentifierParameters(ClaimsIdentity claimsIdentity, string uniqueClaimTypeIdentifier)
        {
            ...
            {
                var matchingClaim = claims.SingleOrDefault(claim => String.Equals(uniqueClaimTypeIdentifier, claim.ClaimType, StringComparison.Ordinal));
                
                if (matchingClaim == null || String.IsNullOrEmpty(matchingClaim.Value))
                {
                    throw new InvalidOperationException(String.Format(CultureInfo.CurrentCulture, StringResources.ClaimUidExtractor_ClaimNotPresent, uniqueClaimTypeIdentifier));
                }
            ...
            // claim.
            var nameIdentifierClaim = claims.SingleOrDefault(claim => String.Equals(NameIdentifierClaimType, claim.ClaimType, StringComparison.Ordinal));
            var identityProviderClaim = claims.SingleOrDefault(claim => String.Equals(IdentityProviderClaimType, claim.ClaimType, StringComparison.Ordinal));
            
            if (nameIdentifierClaim == null || String.IsNullOrEmpty(nameIdentifierClaim.Value)
                || identityProviderClaim == null || String.IsNullOrEmpty(identityProviderClaim.Value))
            {
                throw new InvalidOperationException(StringResources.ClaimUidExtractor_DefaultClaimsNotPresent);
            }
