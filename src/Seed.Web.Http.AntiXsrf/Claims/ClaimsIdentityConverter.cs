...
namespace Seed.Web.Http.AntiXsrf.Claims
{
    // Can convert IIdentity instances into our ClaimsIdentity wrapper.
    internal sealed class ClaimsIdentityConverter
    {
        private static readonly MethodInfo ClaimsIdentityTryConvertOpenMethod = 
            typeof(ClaimsIdentity).GetMethod("TryConvert", BindingFlags.Static | BindingFlags.NonPublic | BindingFlags.Public);
        
        private static readonly ClaimsIdentityConverter DefaultConverter = new ClaimsIdentityConverter(GetDefaultConverters());
        ...
        public ClaimsIdentity TryConvert(IIdentity identity)
        {
            ...
            {
                var result = _converters[i](identity);
        ...
        private static void AddToList(IList<Func<IIdentity, ClaimsIdentity>> converters, Type claimsIdentityType, Type claimType)
        {
            ...
            var tryConvertClosedMethod = 
                ClaimsIdentityTryConvertOpenMethod.MakeGenericMethod(claimsIdentityType, claimType);

            var converter = (Func<IIdentity, ClaimsIdentity>)
                    Delegate.CreateDelegate(typeof (Func<IIdentity, ClaimsIdentity>), tryConvertClosedMethod);
        ...
        private static Func<IIdentity, ClaimsIdentity>[] GetDefaultConverters()
        {
            ...
            {
                var claimsIdentityType = Type.GetType("Microsoft.IdentityModel.Claims.IClaimsIdentity, Microsoft.IdentityModel, Version=3.5.0.0, Culture=neutral, PublicKeyToken=31bf3856ad364e35");
                var claimType = Type.GetType("Microsoft.IdentityModel.Claims.Claim, Microsoft.IdentityModel, Version=3.5.0.0, Culture=neutral, PublicKeyToken=31bf3856ad364e35");
